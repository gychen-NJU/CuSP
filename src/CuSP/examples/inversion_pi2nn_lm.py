...
print_table(
    ...
    [[name, unit, f"{truth:.6g}", f"{guess:.6g}", f"{lm:.6g}",
      f"{abs(lm - truth) / abs(truth):.3e}"]
     for name, unit, truth, guess, lm in
     zip(PARAM_NAMES, PARAM_UNITS, TRUTH, network_params, params_lm)])
print()
print(f"chi2/dof  PI2NN guess  : {merit_start:.4e}")
...
plot_path = HERE / "inversion_pi2nn_lm.png"
plot_comparison(
    plot_path,
    "PI2NN initial guess -> Levenberg-Marquardt  (SDO/HMI Fe I 6173 A)",
