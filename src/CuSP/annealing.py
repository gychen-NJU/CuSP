    ...
    def run(self, step, temperature, **kwargs):
        ...
        if self.history is None:
            ...
            if record_choice is not None:
                if 'record_choice' not in self.history:
                    self.history['record_choice'] = dict()
...
class AdamLocalSearch:
    ...
    def local_search(self,x,e,do_ls=None,**kwargs):
        ...
        if torch.any(do_ls):
            x_init = x.clone()
            e_init = e.clone()
            x_init, e_init = self.minimizer(x_init, e_init, self.ls_max_iter,**kwargs)
...
class BatchAdam:
    ...
    def __init__(self,lower,upper,func,**kwargs):
        ...
        self.max_batches = kwargs.get('max_batches',int(1e10))
    ...
    def __call__(self,x,e,max_iter,**kwargs):
        ...
        if n_batch<=self.max_batches:
            for t in range(1,max_iter+1):
                ...
                et = self.func(xt,**kwargs).to(self.device)
                ...
                # print(f"et device: {et.device} | e_best device: {e_best.device} | xt device: {xt.device}")
...
def DualAnnealing(func,bounds,args=(),maxiter=1000,adam=dict(),initial_temp=5230.,
    ...
    """
    # print('DualAnnealing@maxiter:',maxiter)
    ...
    while not need_to_stop:
        for i in range(maxiter):
            # print('DualAnnealing@iteration:', i)
